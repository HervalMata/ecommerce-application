                                        ...
                                        <div class="img-big-wrap">
                                            <div>
                                                <a href="https://via.placeholder.com/176" data-fancybox="">
                                                    <img src="https://via.placeholder.com/176" alt=""></a>
                                            </div>
                                        </div>
                                    @endif
                                    @if($product->images->count() > 0)
                            ...
                            <aside class="col-sm-7">
                                <article class="p-5">
                                    ...
                                    <form action="{{ route('product.add.cart') }}" method="POST" role="form" id="addToCart">
                                                        ...
                                                        @if($attributeCheck)
                                                            ...
                                                            <dd>
                                                                <select class="form-control form-control-sm option" style="width: 180px;" name="{{ strtolower($attribute->name) }}">
                                                                    <option data-price="{{ $attributeValue->$attribute_id == $attribute->id }}"
                                                                            value="{{ $attributeValue->value }}"> {{ ucwords($attributeValue->value . ' +' . $attributeValue->price) }}</option>
                                                                    @endif
                                                                    @endforeach
                                                                </select>
                                                            </dd>
                                        ...
                                        <hr>
                                        <button type="submit" class="btn btn-success"><i class="fas fa-shopping-cart"></i> Adicionar para o carrinho</button>
                                    </form>
                                </article>
                            </aside>
                        </div>
